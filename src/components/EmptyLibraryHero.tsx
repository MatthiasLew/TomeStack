    ...
    <div className="card-glass rounded-3xl p-8 sm:p-12 text-center border border-brand-500/30 bg-gradient-to-b from-gray-900/90 via-gray-950/90 to-brand-950/20 shadow-2xl space-y-6 max-w-3xl mx-auto">
      ...
      <div className="space-y-2">
        ...
        <p className="text-sm sm:text-base text-gray-400 max-w-xl mx-auto leading-relaxed">
          {lang === "pl"
            ? "Wpisz poni�ej nazwisko dowolnego autora na �wiecie (np. George Orwell, Stanis�aw Lem, Stephen King), aby wczyta� jego tomy z Biblioteki Narodowej i doda� do swojej p�ki jednym klikni�ciem!"
            : "Type any author below (e.g. George Orwell, Stanis�aw Lem, Stephen King) to fetch works from the National Library and add them to your shelf with 1 click!"}
        </p>
      </div>
      ...
      <div className="max-w-md mx-auto flex gap-2">
        ...
        <button
          ...
          title={lang === "pl" ? "Wczytaj ca�� bibliografi� autora" : "Import all books"}
        >
          ...
          <span>{lang === "pl" ? "Dodaj ca�� tw�rczo��" : "Import all"}</span>
        </button>
        <button
          ...
          title={lang === "pl" ? "Przegl�daj ksi��ki pojedynczo" : "Browse books"}
        >
          <Search className="w-3.5 h-3.5" />
          <span className="hidden sm:inline">{lang === "pl" ? "Przegl�daj" : "Browse"}</span>
        </button>
      </div>
      ...
      <div className="pt-2">
        <p className="text-xs text-gray-400 font-semibold mb-2.5">
          {lang === "pl"
            ? "? Kliknij autora, aby od razu za�adowa� wszystkie jego ksi��ki do �ledzenia:"
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {POPULAR_AUTHORS.map((author) => (
            <button
              ...
              title={lang === "pl" ? `Wczytaj ca�� bibliografi� ${author}` : `Load all books by ${author}`}
            >
      ...
      <div className="pt-4 border-t border-gray-800/80 flex items-center justify-center gap-4 text-xs text-gray-500">
        <span>{lang === "pl" ? "Chcesz zobaczy� przyk�adowe dane?" : "Want to see sample data?"}</span>
        <button
          onClick={onLoadDemoData}
          className="text-brand-400 hover:text-brand-300 font-semibold underline underline-offset-4 cursor-pointer"
        >
          {lang === "pl" ? "Za�aduj kolekcj� demonstracyjn�" : "Load demo collection"}
        </button>
      </div>
    </div>
  );
